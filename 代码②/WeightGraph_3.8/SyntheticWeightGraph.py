    ...
    def post_deleteEdge(self):
        ...
        for i in self.G.nodes():
            ...
            while node_triangle_count(i,self.G)<triangle_i:
                neighbour_neighbour_1=find_neighbour_neighbour_1(i,self.G)
                if neighbour_neighbour_1!=None and self.originalWeightGraph.triangleNumber>sum(nx.triangles(self.G).values())/3:
                    weight = random_pick(weight_frequency_list, weight_frequency_probabilities)
                    self.G.add_edge(i, neighbour_neighbour_1, weight=weight)
                    # print(str(i)+","+str(neighbour_neighbour_1)+"有"+str(i_j_triangle(self.G,i,neighbour_neighbour_1)))
                    print("add（"+str(i)+","+str(neighbour_neighbour_1)+")one~~~~~~~~~~~~")
                else:break
        for i in self.G.nodes():
            print("node"+str(i))
            while  node_triangle_count(i,self.G)>triangle_i:
                neighbour_neighbour_2=find_neighbour_neighbour_2(i,self.G)
