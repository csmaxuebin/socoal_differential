...
def obtainDensity(nodeStrengthDic,nodeDegreeThreshold,epsilon_density):
    #dc阈值，自己设置
    dc=30
    ...
    for i in range(len(nodeStrengthDic)-1):
        for j in range(i+1,len(nodeStrengthDic)):
            distance=abs(nodeStrengthDic[i+1]-nodeStrengthDic[j+1])+(np.random.laplace(0.0,(4*nodeDegreeThreshold+2)/epsilon_density,1))
            if distance>=0:
                dist[i,j]=distance
                dist[j,i]=distance
            else:
...
# project_dataframe=project.project(unproject_dataframe,20,21)    
# nodeStrengthDic=nodeStrength.obtainNodeStrength(project_dataframe)
# print(nodeStrengthDic)
# gamma=obtainDensity(nodeStrengthDic,20,21,0.5)
# print(gamma)
